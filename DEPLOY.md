...
4. `docker compose -f compose.landing.yaml up -d`. Проверка пути без DNS:
   `curl -k --resolve $LANDING_DOMAIN:443:<адрес> https://$LANDING_DOMAIN/` —
   `-k` нужен только до того, как Caddy получит сертификат: ACME HTTP-01 не
   пройдёт, пока DNS не смотрит сюда.
5. DNS: A и AAAA голого домена → новый хост. `www` уедет CNAME'ом сам, свой
...
6. Секреты Actions для автоматической выкладки: `LANDING_DEPLOY_HOST`,
   ...
   руками.
7. Мониторинг перенастраивать не нужно: `subnet-watch` проверяет каждое имя
