...
Слепок проверяется в прогоне: работа **Vault snapshot** клонирует вольт,
пересобирает его и сверяет с `vault/*.json`. Разойдётся — прогон краснеет и
показывает, что именно, а до сервера правка не доезжает. Так ручная правка
слепка ловится у автора, а не через сутки на боевой альфе.
