...
Аналитика — Google Tag Manager (`GTM-KJHJZW95`), вставка в `<head>` и сразу после
`<body>`. Страница шлёт в `dataLayer` два события: `signup_success` (успешная
заявка — главная конверсия) и `discord_click` (клик по любой ссылке на Discord).
События в GA4 настраиваются в интерфейсе GTM (триггер Custom Event → тег GA4 Event).
