...
export function Library({ look, session, busy, act }: Props) {
  ...
  return (
    <section>
      ...
      <p className="note">
        Агротехника восьми базовых культур лежит здесь для всех: без неё грядка
        показывает симптом, с ней — норму и остаток до неё. Агротехника
        выведенного сорта в Библиотеку не попадает — её знает только автор
        (D-057).
      </p>

      <p className="note">
        Бесплатно деньгами и без условий, но только придя: удалённо Библиотека
        не работает (D-053). Знание не теряется ни при смерти, ни по суду —
        зато переписать его стоит выносливости, и за день сюда ходят не за
        всем списком сразу (D-148).
      </p>
    </section>
  );
}
