...
export function GraphMap({ look, session, busy, act, onEnter }: Props) {
  ...
  return (
    <section className="map-pane">
      ...
      <p className="note">
        {идёт
          ? "Пока идёшь, тебя нет нигде: всё присутственное закрыто (D-107)."
          : look.survey
            ? "Разведчик в поле: тело недоступно, как во сне. Вернуться — кнопкой выше (D-152)."
            : "Идти можно в любой узел: маршрут строится сам, но идётся по рёбрам — по прямой в этом мире не ходят (D-045)."}
      </p>
    </section>
  );
}
...
function Roads({
  ...
  return (
    <div className="row map-bar">
      ...
      <span className="note">
        Дорога кладётся полотном и временем, а без содержания зарастает обратно
        в бездорожье (D-107). Обоз по бездорожью не идёт.
      </span>
    </div>
  );
}
