...
export function Sidebar({ look, session, busy, act, книга }: Props) {
  ...
  return (
    <aside className="sidebar">
      ...
      <p className="note">
        Сайдбар — это Сеть: работает откуда угодно, хоть с дороги. Всё
        присутственное — в основном окне (D-044, D-050).
      </p>
    </aside>
  );
}
...
function Character({ look, session, busy, act }: Props) {
  ...
  return (
    <div>
      ...
      <p className="note">
        Личность бессмертна: имя, счёт и знания переживают тело. Тело — расходник
        с выносливостью, и восстанавливает её сон — офлайн, по фактическому
        времени; спящий недоступен для присутственного (D-012, D-091).
      </p>
    </div>
  );
}
...
function Trade({ look, session, busy, act }: Props) {
  return (
    <div>
      ...
      <p className="note">
        Ордер живёт на сервере и управляется отсюда, хоть с другой планеты.
        Товар при этом лежит в терминале физически (D-047).
      </p>
    </div>
  );
}
  ...
  return (
    <section>
      ...
      <div className="row">
        ...
        <span className="note">
          Залога нет: лимит выдаёт труд — оборот продаж, возвращённые кредиты и
          доверие (D-173). Гражданам занимает их город со своей маржой; погашение
          уходит в резерв столицы (D-175).
        </span>
      </div>
    </section>
  );
