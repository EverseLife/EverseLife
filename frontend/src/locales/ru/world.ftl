...
ui-map-search-rule = Разведчик уходит сам и до возвращения недоступен, как во сне. Без сил в поле не выходят: сначала поесть или поспать. Нашёл — там и остаётся; чем дальше от города находка, тем длиннее к ней дорога. Лес попадается и сам собой, но заказанный ищется дольше. Кнопка стоит на том слое, куда ляжет находка: участок и помещение — в застройке, место, жила и лес — на поверхности планеты.
ui-map-forecast = заход отсюда: { $term } · шанс { $chance }% · { $price } выносливости на выход
...
ui-ship-reserve = Сверх подъёма на спуск обратно уйдёт ещё { $kept }.
