...
ui-ship-no-orbit = Отсюда не подняться: вокруг этой планеты нет орбиты.
ui-ship-climb-to = Орбита: { $planet }
ui-ship-orbit-high = Орбита слишком высока, чтобы садиться: сначала проложите курс к этой же планете — автопилот опустит корабль на стоянку.
ui-ship-no-thrust = тяги нет вовсе: поставьте двигатель
ui-ship-leg-cost = { $hours } ч · { $fuel } топлива
ui-ship-ascend = Подняться на орбиту
...
ui-ship-fly = Лететь
ui-ship-fly-hint = переход идёт к планете; космодром выбирается уже на её орбите
...
ui-factory-cut-off = Узел отключён за неуплату: машины не работают, пока долг не закрыт; пропущенные часы они не наверстают. Счёт — в сайдбаре, во вкладке «хозяйство».
ui-factory-frozen = { $heat ->
        [true] Узел раскалён: машины здесь не работают и не заработают — остудить узел нельзя.
       *[false] Узел промёрз: машины не работают, пока узел не обогрет; пропущенные часы они не наверстают. Тепло даёт «{ NAME("heat_plant") }», «{ NAME("heater") }» или «{ NAME("brazier") }» с топливом.
    }
