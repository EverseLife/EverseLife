...
ui-intro-go = Начать
ui-intro-again = Открыть снова — «кто вы» во вкладке «аккаунт».

## Регистрация в пять шагов (D-187)
...
ui-register-lines-empty = Ни одной линии: мир ещё не собран. Попробуйте позже.
...
ui-doors-no-city = Вокруг этой двери города нет: некому платить подъёмные, не во что записывать и нечего удерживать с продаж.
...
ui-entry-globe-doors-hint = Крупные точки — принтеры, у которых вас могут напечатать: чем больше точка, тем больше в городе граждан. Выберите точку — откроется карточка её города; глобус поворачивается перетаскиванием.
