...
ui-place-plot-wild = Земля за городом ничья: бумагу на владение выдаёт город, а здесь его нет. Работать и строить тут может всякий — поставленное принадлежит поставившему.
...
ui-place-floor-install = Установить
ui-place-floor-install-hint = поднять с пола и поставить в здание: займёт место
...
ui-place-convoy-unharness-rule = Обоз останется здесь с грузом; по бездорожью и тропинке он не идёт.
...
ui-stand-scheme-about = Окно схемы корабля: из какой тары машины берут и в какую льют — здесь проводят линии и называют тару.
...
ui-holdings-home = Домой
ui-holdings-home-hint = идти в этот узел; маршрут строится сам
ui-holdings-home-here = вы здесь
ui-holdings-no-way = Туда пути нет: узлы не связаны рёбрами.
