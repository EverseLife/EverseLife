...
ui-ship-feed-hint = A port with nothing ticked draws nothing and pours nowhere. Tick the vessels; the order of ticking is the order the port draws from them or fills them.
ui-ship-feed-none = No machine with lines aboard: an engine, the life support system, an electrolyser, an automatic reactor and a hydroponic unit appear here once they stand in a compartment.
...
ui-ship-scheme = Ship scheme
ui-ship-scheme-rule = lines from machines to vessels
ui-ship-scheme-hint = Lanes are the compartments in the order they were laid: machines on the left, vessels on the right. A line runs from a port's dot to a vessel — drag it, or press the port and then the vessel. Pressing a line opens its port; pressing a vessel with no port chosen opens its name.
...
ui-ship-scheme-none = No machine with ports aboard: an engine, the life support system, an electrolyser, an automatic reactor and a hydroponic unit appear here once they stand in a compartment.
...
ui-ship-scheme-way-out-note = Pours into the vessels in line order; when all of them are full the machine stands.
ui-ship-scheme-way-vent-note = Pours into the vessels in line order; what does not fit goes overboard, and the machine does not stand for it.
ui-ship-scheme-no-line = { $way ->
        [in] no line: the port draws nothing
        [vent] no line: all of it goes overboard
       *[other] no line: the machine has nowhere to pour, and it stands
    }
...
ui-ship-scheme-unline = take off the line
ui-ship-scheme-done = done
ui-ship-scheme-name-label = Vessel name
ui-ship-scheme-name-clear = remove the name
ui-ship-scheme-stall-power = standing: the hull's batteries are flat
ui-ship-scheme-stall-dry = standing: the “{ $goods }” line is dry
ui-ship-scheme-stall-full = standing: the vessels on the “{ $goods }” line are full
