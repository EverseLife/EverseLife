...
say "0. Where the rename stopped last time"
# The script is meant to be run again after a stumble, so nothing below assumes
# a starting point: it looks at the database and does only what is left.
if docker compose exec -T postgres psql -U "$NEW" -d postgres -tAc 'select 1' > /dev/null 2>&1; then
    ROLE=$NEW
else
    ROLE=$OLD
fi
if docker compose exec -T postgres psql -U "$ROLE" -d postgres -tAc         "select 1 from pg_database where datname='$NEW'" | grep -q 1; then
    DB=$NEW
else
    DB=$OLD
fi
echo "  role: $ROLE, database: $DB"

say "1. A dump first: the net under everything below"
mkdir -p ~/backup
DUMP=~/backup/before-rename-$(date +%F-%H%M).dump
docker compose exec -T postgres pg_dump -U "$ROLE" -Fc "$DB" > "$DUMP"
[ -s "$DUMP" ] || { echo "the dump is empty -- stopping"; exit 1; }
echo "dump: $DUMP ($(du -h "$DUMP" | cut -f1))"

say "2. The database and its role take the new name"
...
if [ "$DB" = "$OLD" ]; then
    docker compose exec -T postgres psql -U "$ROLE" -d postgres -v ON_ERROR_STOP=1         -c "ALTER DATABASE $OLD RENAME TO $NEW;"
    echo "  database: $OLD -> $NEW"
else
    echo "  database is already $NEW"
fi
if [ "$ROLE" = "$OLD" ]; then
    # A session user cannot rename itself, so a helper superuser does it and
    # then goes away. Passwords survive: Postgres 14 and up store them as
    # scram, which does not depend on the name.
    docker compose exec -T postgres psql -U "$OLD" -d "$NEW" -v ON_ERROR_STOP=1         -c "CREATE ROLE rename_helper SUPERUSER LOGIN;"
    docker compose exec -T postgres psql -U rename_helper -d "$NEW" -v ON_ERROR_STOP=1         -c "ALTER USER $OLD RENAME TO $NEW;"
    docker compose exec -T postgres psql -U "$NEW" -d "$NEW" -v ON_ERROR_STOP=1         -c "DROP ROLE rename_helper;"
    echo "  role: $OLD -> $NEW"
else
    echo "  role is already $NEW"
fi

say "3. The old stack stops, volumes stay"
docker compose down > /dev/null

say "4. Volumes are copied under the new project's names"
...
say "5. The directory moves and `.env` takes the new names"
