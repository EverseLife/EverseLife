...
- `backend/src/constants/registry_*.py` in the game repo -- the engine's declarations (`Num`, `Span`, `Table`, `Flag`, `Text`, `Formula`), one section per roadmap stage behind the door `registry.py`, checked against `build/` at startup.
