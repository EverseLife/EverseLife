...
from octoverse.models.event import Event
from octoverse.models.identity import Knowledge
from octoverse.models.inventory import Item
from octoverse.units import amount_float
...
async def test_появление_предмета_обязано_иметь_основание(session: AsyncSession) -> None:
    ...
    await world.grant_item(
        session, container, "Руда", amount=12.5, quality=60, origin="сценарий отладки"
    )
    await session.commit()

    item = (await session.execute(select(Item))).scalar_one()
    assert item.type_key == "Руда"
