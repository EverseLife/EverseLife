...
The city as an institution (D-154), the household meter (D-149) and machine
occupancy (D-150). The migration is written by hand, not autogenerated: other
work was going on in the model tree at the same time, and `--autogenerate`
would have swept unrelated unmigrated tables into this file.
...
def upgrade() -> None:
    ...
    #: Machine occupancy (D-150): one person works at a machine, and that is a
    #: property of the machine itself, not a separate queue table.
