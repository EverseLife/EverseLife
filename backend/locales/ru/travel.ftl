...
ship-hours-are-a-flyby = на { NUMBER($hours) } ч ползунок предлагает не прямую дугу, а пролёт: { PLANET($planet) }. Выберите эту точку на ползунке заново
ship-no-arc = на { NUMBER($hours) } ч небо дуги не даёт: всякая срезает корону звезды. Выберите другое время на ползунке
ship-no-flyby = пролёт: { PLANET($planet) } — на { NUMBER($hours) } ч его на ползунке нет: небо его не предлагает или повернулось, пока выбирали время. Выберите время на ползунке заново
