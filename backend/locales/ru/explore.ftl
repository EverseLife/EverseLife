...
## Путь к известному узлу (дополнение к D-321 от 2026-09-12): тот же поход, но
## цель — стоящий узел, и отказ говорит о пути, а не о разведке. Досягаемость
## пути — от края своей земли до края чужой (дополнение от 2026-09-18).
path-too-near = слишком близко для пути: до «{ $node }» { $metres } м, а проложить путь отсюда можно не ближе { $near } м
path-too-far = слишком далеко для пути: до «{ $node }» { $metres } м, а к узлу такой величины путь отсюда кладут не дальше { $far } м
path-into-water = путь до «{ $node }» лёг бы через воду: по воде пути не кладут, реку переходят бродом
path-crosses-way = путь до «{ $node }» пересёк бы уже проложенный: пути сходятся только в узлах и друг друга не пересекают
path-through-node = путь до «{ $target }» прошёл бы сквозь узел «{ $node }»: путь не проходит сквозь другие узлы
