...
# Руины Предтеч в черте города — земля города, но не участок (D-356).
city-land-ruin = «{ $node }» — руины Предтеч, а не участок: город их не раздаёт
# Раздают только пустую землю (D-356): на находке, которую взял тракт или
# накрыла граница, может стоять чужая работа, начатая на дикой земле.
city-land-not-vacant = участок не пустой: город раздаёт только землю без здания, оборудования и жилы, а делянки и стройки на ней — только того, кому её отдают
...
city-found-name-taken = город с именем «{ $name }» уже есть
city-found-name-in-the-net = имя «{ $name }» уже носит канал в Сети, а город зовётся так же, как его канал
...
# $weather — климат планеты: frost (мерзлота) или heat (пекло, узел не остудить).
city-hall-frozen = { $weather ->
        [heat] узел «{ $node }» раскалён: администрация закрыта — остудить узел нельзя
       *[frost] узел «{ $node }» промёрз: администрация закрыта, пока его не обогреют
    }
...
city-leave-in-debt = за вами непогашенный кредит: город отвечает за своих заёмщиков, и выйти из гражданства можно только рассчитавшись
city-found-while-citizen = город основывает тот, кто нигде не состоит: сначала выйдите из гражданства прежнего города
...
estate-build-on-ice = на ледяном поле не строят: лёд ползёт и трескается под стенами
...
estate-land-ruin = «{ $node }» — руины Предтеч, а не участок: город их не продаёт
estate-land-not-vacant = участок не пустой: город продаёт только землю без здания, оборудования, жилы, чужих делянок и строек
