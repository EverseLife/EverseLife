...
farm-too-cold = «{ $culture }» сейчас здесь вымерзнет: ночь опускается до { $night }°
farm-too-hot = «{ $culture }» сейчас здесь сгорит: полдень доходит до { $noon }°
...
farm-on-ice = здесь ледяное поле: на льду не межуют и не сеют
...
# Скафандр не снимают и не заменяют вещью, которая не скафандр, там, где он один даёт дышать (D-343).
oxygen-suit-stays-on = в «{ $node }» нечем дышать: «{ NAME($suit) }» снимают на борту или там, где есть воздух
# В пути — снаружи (D-343): тело на отрезке ещё числится там, откуда вышло.
oxygen-suit-stays-on-road = дорога в «{ $node }» идёт снаружи, где нечем дышать: «{ NAME($suit) }» снимают на борту или там, где есть воздух, а не в пути
...
battery-not-here = этот аккумулятор здесь не стоит
battery-charge-in-hands = «{ NAME($goods) }» в руках не заряжают: сначала поставьте в здание
battery-charge-lying = «{ NAME($goods) }» лежит, а заряжают только то, что стоит: поставьте в здание
...
rig-node-not-yours = участок не ваш: установку ставят на своей или ничьей земле
rig-not-here = установка не здесь: бункер вывозят ногами
rig-not-yours = чужая установка: вывоз — по договору с хозяином
# Переезд на другую жилу (D-314): порода читается с той жилы, на которой машина стоит сейчас.
# Бункер бывает и нефтяным, поэтому «не пуст», а не «осталась руда».
rig-hopper-not-empty = бункер установки «{ NAME($goods) }» не пуст: на другую жилу она переезжает только пустой
rig-machine-gone = установки здесь больше нет: не стало и того, что было в бункере
rig-machine-elsewhere = установки здесь нет: ни во дворе, ни в руках — бункер вывозят у машины
