...
liquid-mixed = в «{ NAME($vessel) }» уже налито «{ NAME($have) }»: две жидкости в одной таре не мешают

# --- линии борта (engine/ship/lines.py, D-288) --------------------------------

line-no-such-port = у «{ NAME($goods) }» нет порта «{ $port }»
line-machine-not-aboard = «{ NAME($goods) }» не установлена на этом корабле: линию тянут от установленной машины
line-vessel-not-aboard = «{ NAME($goods) }» не установлена на этом корабле: на линии стоит только установленная тара
