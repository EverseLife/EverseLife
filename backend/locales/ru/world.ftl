...
liquid-not-vent = в «{ NAME($vessel) }» налито «{ NAME($have) }»: выпускают и сжигают только сбросной газ, остальное переливают в другую тару
liquid-vent-nowhere = «{ NAME($goods) }» из «{ NAME($vessel) }» деть некуда: под небом с воздухом его { $aboard ->
        [true] с борта не выпускают, а факела на корабле нет
