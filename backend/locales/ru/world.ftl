...
farm-fertilize-sown = на «{ $plot }» растёт культура: удобряют землю, а не грядку — подкормка придёт с пятью решениями ухода
...
farm-recut-sown = перекроить можно только незасеянное
