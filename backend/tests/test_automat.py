...
The world's pass over every machine at once is in `test_automat_tick.py`.
"""
...
async def test_a_node_cut_off_for_debt_stops_its_automat_until_the_bill_is_paid(
    through: str, session: AsyncSession, constants: Constants, catalog: Catalog
) -> None:
    """Did not pay -- disconnected, and the machines of the node do not work
    until the bill is paid (D-149): the automat as much as a bench. The stopped
    hours pass as at any stop -- worn through, not worked, and not given back
    by the payment. Both doors: a command advancing the one machine, and the
    world's tick, which asks for the energy without a lock and draws it last."""
    node, yard, identity, body, machine = await _factory_floor(session, constants)
    ...
    assert await settle(stood) == 0, "a node in debt runs no machine"
    assert row.counted_at == stood and row.recipe_key == NAILS
    assert float(row.backlog) == 0
    ...
    await utility.pay(session, constants, identity, node)
