...
async def test_written_carrier_is_a_position_of_its_own(
    ...
    assert fill.order.type_key == carrier
...
async def test_synonym_trades_under_the_world_name(
    ...
    stuff = {material.name for material in book.materials}
    synonym, canonical = next((name, real) for name, real in book.synonyms.items() if real in stuff)
    node = await _city(session)
    tier = market.tier_of(constants, 65)
