...
async def test_признак_не_переспросить(session: AsyncSession, constants: Constants) -> None:
    """Иначе среднее по прочтениям выдаёт скрытое число с любой точностью.

    Свод меняется только от удара и крепи — значит и признак обязан меняться
    только вместе с ними, а не при каждом взгляде (D-143).
    """
    _, vein, body = await _забой(session)
    сессия = await mining.start(session, constants, body, vein)

    первый = await mining.sight(session, constants, сессия)
    повторные = {(await mining.sight(session, constants, сессия)).sign for _ in range(20)}
    assert повторные == {первый.sign}, "признак обязан быть одним и тем же до удара"

    после_удара = await mining.swing(session, constants, сессия)
    ещё = {(await mining.sight(session, constants, сессия)).sign for _ in range(5)}
    assert ещё == {после_удара.sign}
...
async def test_обрушение_стоит_всей_сессии(
    ...
    #: делённые на срок службы этого качества — хорошая кирка держится дольше
    #: (`engine.wear`).
    from src.engine import wear

    удар = constants[R.WEAR_TOOL_PER_SESSION] + constants[R.MINE_COLLAPSE_WEAR]
    ожидаемый = удар / wear.life_factor(constants, 50)
    await session.refresh(кирка)
    assert float(кирка.condition) == pytest.approx(100 - ожидаемый, abs=0.01)
...
async def test_обрушение_иногда_ранит(session: AsyncSession, constants: Constants) -> None:
    ...
    #: Бросок подобран так, чтобы разминуться со смертью и попасть в рану:
    #: десятая доля шкалы выше `mine.collapse_death_chance`, но ниже
    #: `mine.collapse_wound_chance`.
    невезучий = random.Random()
    невезучий.uniform = lambda a, b: a + (b - a) / 10  # noqa: ARG005
    for _ in range(100):
        вид = await mining.swing(session, constants, сессия, rng=невезучий)
        if вид.state is SessionState.COLLAPSED:
            break
    await session.commit()
...
async def test_обрушение_иногда_убивает(
    session: AsyncSession, constants: Constants
) -> None:
    """Среда — единственный источник смерти в альфе: бой заморожен (D-111).

    «Обвал и газ убивают одинаково новичка и старожила» (08-danger), поэтому
    самый неудачный бросок обязан кончаться гибелью, а не раной.
    """
    node, vein, body = await _забой(session)
    карман = await world.body_container(session, body)
    await world.grant_item(
        session, карман, "Уголь", amount=100, quality=50, origin="сценарий теста"
    )
    сессия = await mining.start(session, constants, body, vein)

    несчастный = random.Random()
    несчастный.uniform = lambda a, b: a  # noqa: ARG005 — всегда нижняя граница
    for _ in range(100):
        вид = await mining.swing(session, constants, сессия, rng=несчастный)
        if вид.state is SessionState.COLLAPSED:
            break
    await session.commit()

    assert body.state is BodyState.DEAD and body.died_at is not None
    #: Раны мёртвому не выписывают: он уже не работает медленнее.
    ран = await session.scalar(
        select(func.count()).select_from(Wound).where(Wound.body_id == body.id)
    )
    assert ран == 0
    #: Часть носимого осталась на месте гибели (D-033, `death.salvage_ratio`).
    from src.models.inventory import Item
    ...
    assert осталось, "мёртвый оставляет треть, а не ничего"
...
# --- выносливость (ограничение нуля) ----------------------------------------


async def test_с_нулём_выносливости_не_бьют(
    session: AsyncSession, constants: Constants
) -> None:
    """Тело с нулём не добывает: удар стоит сил, и без них жила закрыта."""
...
async def test_с_нулём_выносливости_сессия_не_открывается(
    session: AsyncSession, constants: Constants
) -> None:
    from decimal import Decimal
