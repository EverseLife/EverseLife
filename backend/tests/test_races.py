...
async def _forecaster(session: AsyncSession, name: str) -> uuid.UUID:
    ...
    body = await world.print_body(session, identity, node)
    yard = await world.node_container(session, node)
    await world.grant_item(session, yard, "Кузница", quality=60, origin="тест")
    pocket = await world.body_container(session, body)
    await world.grant_item(session, pocket, "Слиток железа", amount=10, quality=80, origin="тест")
...
async def test_forecasts_write_nothing(
    ...
    async with factory() as db, db.begin(), _writes_forbidden(db):
        plan = await _craft_plan({"identity_id": who}, db, {"output": "Гвозди", "units": 3})
...
async def test_forecasts_make_no_yard_in_a_place_without_one(
    ...
    async with factory() as db, db.begin(), _writes_forbidden(db):
        ...
        with pytest.raises(craft.NoStation):
            await _craft_plan(who, db, {"output": "Гвозди", "units": 3})
...
async def test_forecast_does_not_wait_for_the_body_lock(
    ...
    async def plan() -> dict:
        async with factory() as db, db.begin():
            return await _craft_plan({"identity_id": who}, db, {"output": "Гвозди", "units": 3})
