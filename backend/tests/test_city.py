...
async def _printer(session: AsyncSession, node, *, standing: bool):
    ...
    the two cases are the two sides of this rule: what the city **has** is a
    machine standing, and what a hand may try to put up is one lying.
    """
...
async def test_the_city_puts_the_printer_up_and_not_the_plot_holder(
    ...
    **node**, and on a bought plot that right is the holder's. Without this the
    ...
    plot = await world.create_node(
        session, f"{core.key}.plot", "Участок", area_m2=100, parent=core.parent_id and None
    )
