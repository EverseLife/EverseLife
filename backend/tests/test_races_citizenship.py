...
lends, and one does not leave it owing. Two sessions on the same citizenship --
one borrowing, one walking out -- and the pair of rules is worth exactly as
much as the row lock under it: without it the bank reads a citizenship the exit
is about to delete, and the city is left answering on its line with the capital
for somebody who is no longer its citizen.

Its own file rather than a fifth section of `test_races.py`: that one is about
the arithmetic of money, and this is about the record that decides who may
touch it. The city's line -- the other remainder D-281 turned into a hard
ceiling -- is raced there, beside the other sums
(`test_two_citizens_do_not_lie_twice_on_the_same_line`).
"""
